 ...
 ** 电平控制简化_宏定义
 ***********************************************/
#define SCL_L      (I2C_MONI_SCL_GPIO7->BSRR = (I2C_MONI_SCL_PIN7 << 16))
#define SCL_H      (I2C_MONI_SCL_GPIO7->BSRR =  I2C_MONI_SCL_PIN7)
#define SDA_L      (I2C_MONI_SDA_GPIO7->BSRR = (I2C_MONI_SDA_PIN7 << 16))
#define SDA_H      (I2C_MONI_SDA_GPIO7->BSRR =  I2C_MONI_SDA_PIN7)
#define SDA_READ   ((I2C_MONI_SDA_GPIO7->IDR & I2C_MONI_SDA_PIN7)?1:0)       // 不用改成输入模式就能读
...
*备  注：
*****************************************************************************/
void IICSoft_Init7(void)
{
    // 使能SCL引脚端口时钟
    if(I2C_MONI_SCL_GPIO7 == GPIOA)   __HAL_RCC_GPIOA_CLK_ENABLE();
    if(I2C_MONI_SCL_GPIO7 == GPIOB)   __HAL_RCC_GPIOB_CLK_ENABLE();

    // 使能SDA引脚端口时钟
    if(I2C_MONI_SDA_GPIO7 == GPIOA)  __HAL_RCC_GPIOA_CLK_ENABLE();
    if(I2C_MONI_SDA_GPIO7 == GPIOB)  __HAL_RCC_GPIOB_CLK_ENABLE();



    // 配置引脚工作模式: 开漏输出
    GPIO_InitTypeDef G;
    G.Pin   = I2C_MONI_SCL_PIN7;
    G.Mode  = GPIO_MODE_OUTPUT_OD;
    G.Speed = 50000000 ;
    HAL_GPIO_Init(I2C_MONI_SCL_GPIO7, &G);
    delayUs(2);

    G.Pin   = I2C_MONI_SDA_PIN7;
    HAL_GPIO_Init(I2C_MONI_SDA_GPIO7, &G);
    delayUs(2);
 ...
 * 备  注： 注意，时序有点特别
*****************************************************************************/
uint8_t IICSoft_ReadByte7(uint8_t slave, uint8_t addr, uint8_t *buf)
{
    start ();                     // 起始信号
 ...
 * 备  注： 注意，时序有点特别
*****************************************************************************/
uint8_t IICSoft_ReadBueffer7(uint8_t slave, uint8_t addr, uint8_t *buf, uint8_t len)
{
    start ();                     // 起始信号
    sendByte (slave<<1 | 0);      // 从机地址,　写方向 , 0写1读
    if(waitForAck())              // 等待ACK
 ...
 * 备  注： 时序与读是不同的
*****************************************************************************/
uint8_t IICSoft_WriteByte7(uint8_t slave, uint8_t addr, uint8_t data)
{
    start ();                     // 起始信号
    sendByte (slave<<1 | 0);      // 从机地址,　写方向 , 0写1读
    if(waitForAck())
...
// 往从机写多个字节数据
uint8_t IICSoft_WriteBuffer7(uint8_t slave, uint8_t addr, uint8_t *buf, uint8_t len)
{
    start ();                     // 起始信号
    sendByte (slave<<1 | 0);      // 从机地址,　写方向 , 0写1读
    if(waitForAck())
...
uint8_t detectMagnet7(void)
{
    ...
    IICSoft_ReadByte7(_ams5600_Address,_stat,&magStatus);
    detectMag7++;
...
int16_t getRawAngle7(void)
{
    int getRaw7=0;
    uint8_t Raw7[2]={0,0};
    IICSoft_ReadBueffer7(_ams5600_Address, _raw_ang_hi, (uint8_t *) Raw7, 2);
    getRaw7 = (Raw7[0] << 8 | Raw7[1]);
    return getRaw7;
...
******************************************************/
float convertRawAngleToDegrees7(int16_t newAngle)
{
    /* Raw data reports 0 - 7095 segments, which is 0.087 of a degree */
...
float Programe_Run7(void)
{
    float degree7=0;
    float rawdata7=0;

    rawdata7 = getRawAngle7();
    degree7 = convertRawAngleToDegrees7(rawdata7);
