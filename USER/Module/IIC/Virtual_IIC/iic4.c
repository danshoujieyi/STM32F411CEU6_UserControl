...
void IICSoft_Init4(void)
{
    ...
    // 配置引脚工作模式: 开漏输出
    GPIO_InitTypeDef G;
    G.Pin   = I2C_MONI_SCL_PIN4;
    G.Mode  = GPIO_MODE_OUTPUT_OD;
    G.Speed = 50000000 ;
    HAL_GPIO_Init(I2C_MONI_SCL_GPIO4, &G);
    delayUs(2);

    G.Pin   = I2C_MONI_SDA_PIN4;
    HAL_GPIO_Init(I2C_MONI_SDA_GPIO4, &G);
    delayUs(2);
 ...
 * 备  注： 注意，时序有点特别
*****************************************************************************/
uint8_t IICSoft_ReadByte4(uint8_t slave, uint8_t addr, uint8_t *buf)
{
    start ();                     // 起始信号
 ...
 * 备  注： 注意，时序有点特别
*****************************************************************************/
uint8_t IICSoft_ReadBueffer4(uint8_t slave, uint8_t addr, uint8_t *buf, uint8_t len)
{
    start ();                     // 起始信号
    sendByte (slave<<1 | 0);      // 从机地址,　写方向 , 0写1读
    if(waitForAck())              // 等待ACK
 ...
 * 备  注： 时序与读是不同的
*****************************************************************************/
uint8_t IICSoft_WriteByte4(uint8_t slave, uint8_t addr, uint8_t data)
{
    start ();                     // 起始信号
    sendByte (slave<<1 | 0);      // 从机地址,　写方向 , 0写1读
    if(waitForAck())
...
// 往从机写多个字节数据
uint8_t IICSoft_WriteBuffer4(uint8_t slave, uint8_t addr, uint8_t *buf, uint8_t len)
{
    start ();                     // 起始信号
    sendByte (slave<<1 | 0);      // 从机地址,　写方向 , 0写1读
    if(waitForAck())
...
uint8_t detectMagnet4(void)
{
    ...
    IICSoft_ReadByte4(_ams5600_Address,_stat,&magStatus);
    detectMag4++;
...
int16_t getRawAngle4(void)
{
    int getRaw4=0;
    uint8_t Raw4[2]={0,0};
    IICSoft_ReadBueffer4(_ams5600_Address, _raw_ang_hi, (uint8_t *) Raw4, 2);
    getRaw4 = (Raw4[0] << 8 | Raw4[1]);
    return getRaw4;
...
******************************************************/
float convertRawAngleToDegrees4(int16_t newAngle)
{
...
float Programe_Run4(void)
{
    float degree4=0;
    float rawdata4=0;

    rawdata4 = getRawAngle4();
    degree4 = convertRawAngleToDegrees4(rawdata4);
